import { useEffect, useRef, useState } from "react";
import { useParams } from "react-router";
import data from "../../data/productData.json"
import type { ProductType } from "../../types/product";
import { FaPlus } from "react-icons/fa";
import { FiMinus } from "react-icons/fi";

function ProductDetail() {
    const [productFound, setProductFound] = useState<boolean>(false);
    const [activeProduct, setActiveProduct] = useState<ProductType | any>({});
    const [productCount , setProductCount] = useState<number>(0);

    const ImagesDiv = useRef<HTMLDivElement>(null);

    const { id } = useParams();

    useEffect(() => {
        let product: ProductType | undefined = data.find((data: ProductType) => String(data.id) == id);

        if (product) {
            setProductFound(true);
            setActiveProduct(product);
            console.log(activeProduct);
        }
        else {
            console.log("ürün bulunamadı");
        }

    }, [id])

    useEffect(() => {
        if (!ImagesDiv.current || !activeProduct) return;

        const images = ImagesDiv.current.getElementsByTagName("img");
        if (images.length > 1) {
            images[1].classList.add("opacity-100");
        }

    }, [activeProduct]);   // ürün render edildikten sonra çalışır

    /* Aktif resmi seç */
    const selectImg = (e: React.MouseEvent<HTMLImageElement, MouseEvent>) => {
        const images: HTMLCollectionOf<HTMLImageElement> | undefined = ImagesDiv.current?.getElementsByTagName("img");

        if (!images) return;

        for (let i = 0; i < images?.length!; i++) {
            if (images[i].id != "mainImage") {
                images[i].classList.add("opacity-60");
                images[i].classList.remove("opacity-100");
            }
        }

        e.currentTarget.classList.add("opacity-100");
        e.currentTarget.classList.remove("opacity-60");

        images[0].src = e.currentTarget.src; // Ana resmi seçilen resim ile güncelle
    }

    const getStars = () => {
        // Belirtilen sayıda yıldız elementi içeren bir dizi oluşturur.
        // Array.from ile istenen uzunlukta bir dizi yaratıp doğrudan map'leyebiliriz.
        return Array.from({ length: activeProduct.starCount || 0 }, (_, index) => (
            <svg key={index} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                    className="size-6 text-yellow-500">
                    <path fillRule="evenodd"
                        d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z"
                        clipRule="evenodd" />
                </svg>
            )
        )
    };

    return (

        <div className="bg-gray-100">
            <div className="container mx-auto px-4 py-8">
                <div className="flex flex-wrap -mx-4">
                    {/* <!-- Product Images --> */}
                    <div className="w-full md:w-1/2 px-4 mb-8" ref={ImagesDiv}>
                        <img alt="Product" src={productFound ? activeProduct.productImageUrl : undefined}
                            className="w-full h-auto rounded-lg shadow-md mb-4" id="mainImage" />
                        <div className="flex gap-4 py-4 justify-center overflow-x-auto">
                            {
                                productFound && activeProduct.bottomImagesUrl.map((image: string, index: number) => (
                                    <img key={index} src={image} alt="Thumbnail 1"
                                        className="size-16 sm:size-20 object-cover rounded-md cursor-pointer opacity-60 hover:opacity-100 transition duration-300"
                                        onClick={(e) => selectImg(e)} />
                                ))
                            }
                        </div>
                    </div>

                    {/* <!-- Product Details --> */}
                    <div className="w-full md:w-1/2 px-4">
                        <h2 className="text-3xl font-bold mb-2">{productFound && activeProduct.title}</h2>
                        <p className="text-gray-600 mb-4">{productFound && activeProduct.category}</p>
                        <div className="mb-4">
                            <span className="text-2xl font-bold mr-2">{productFound && activeProduct.discountPrice}</span>
                            <span className="text-gray-500 line-through">{productFound && activeProduct.price}</span>
                        </div>
                        <div className="flex items-center mb-4">
                            {getStars()}
                            <span className="ml-2 text-gray-600">{productFound && activeProduct.starCount} Puan</span>
                        </div>
                        <p className="text-gray-700 mb-6">{productFound && activeProduct.description}</p>

                        <div className="mb-6">
                            <h3 className="text-lg font-semibold mb-2">Renkler:</h3>
                            <div className="flex space-x-2">
                                <button
                                    className="w-8 h-8 bg-black rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black"></button>
                                <button
                                    className="w-8 h-8 bg-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300"></button>
                                <button
                                    className="w-8 h-8 bg-blue-500 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"></button>
                            </div>
                        </div>

                        <div className="mb-6 flex items-center gap-x-4">
                            <button onClick={() => setProductCount(productCount > 0 ? productCount - 1 : 0)} className="bg-red-500 text-white px-3 py-2 cursor-pointer">{<FiMinus />}</button>
                            <div className="font-bold text-2xl">{productCount}</div>
                            <button onClick={() => setProductCount(productCount + 1)} className="bg-red-500 text-white px-3 py-2 cursor-pointer">{<FaPlus />}</button>
                        </div>

                        <div className="flex space-x-4 mb-6">
                            <button
                                className="bg-green-600 flex gap-2 items-center text-white px-6 py-2 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    strokeWidth="1.5" stroke="currentColor" className="size-6">
                                    <path strokeLinecap="round" strokeLinejoin="round"
                                        d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                                </svg>
                                Sepete Ekle
                            </button>
                            <button
                                className="bg-gray-200 flex gap-2 items-center  text-gray-800 px-6 py-2 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    strokeWidth="1.5" stroke="currentColor" className="size-6">
                                    <path strokeLinecap="round" strokeLinejoin="round"
                                        d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                                </svg>
                                Favorilere Ekle
                            </button>
                        </div>
                    </div>
                </div>
            </div>

        </div>

    )
}

export default ProductDetail;